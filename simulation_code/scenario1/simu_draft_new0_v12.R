...
for (iter in 1:n.run){if(iter %%1==0) {print(10000+iter)}
  for (k in 1:1){
    for (h in 1:4){
      ...
      index2<-sample(setdiff(c(1:m),index1),size=1550,replace=F)
      index3<-sample(c(index1),size=450,replace=F)
      ...
      ctl0<-c(sample(which(beta1!=0&delta[,1]!=0),size=100,replace=F),
              ...
              sample(which(beta1==0&delta[,1]==0),size=100,replace=F),
              sample(which(beta1!=0&delta[,1]==0),size=50,replace=F))
      ctl<-rep(F,m)
      ctl[ctl0]<-T;sum(ctl)
...
save.image("Data-Case0_v12.RData")
