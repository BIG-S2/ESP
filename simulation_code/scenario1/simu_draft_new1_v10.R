...
for (iter in 1:n.run){if(iter %%1==0) {print(10000+iter)}
  for (k in 1:1){
    for (h in 1:4){
      ...
      ##error matrix 
      #e<-matrix(NA,n,m)
      #sigma2<-rigamma(m,alpha=10,beta=9)
      #for (i in 1:m){
      #  e[,i]<-rnorm(n,mean=0,sd=sqrt(sigma2[i]))
      #}
      e<-matrix(rnorm(n*m),n,m)
      dat=mvrnorm(n = n, rep(0, 2), Sigma=matrix(c(1,xz_cor[h],xz_cor[h],1),2,2))
      x=as.matrix(dat[,1])
      z=as.matrix(dat[,2])
      ##beta matrix: coeffs matrix for x
      #number of features with non-zero beta is 1000
      beta1<-matrix(0,m,1)
      ...
      index2<-sample(setdiff(c(1:m),index1),size=1100,replace=F)
      index3<-sample(c(index1),size=900,replace=F)
...
save.image("Data-Case1_v10.RData")
