...
for (iter in 1:n.run){if(iter %%1==0) {print(10000+iter)}
  for (k in 1:1){
    for (h in 1:4){
      ...
      #1800 features are influenced by the hidden variables 
      delta<-matrix(0,m,1)
      Sigma <- matrix(c(1,0,0,1),2,2)   #B D correlation
      coef<-mvrnorm(n=m,mu=c(0,0),Sigma=Sigma)
      cor(coef[,1],coef[,2])
      index1<-sample(c(1:m),size=500,replace=F)  #sparsity
      index2<-sample(setdiff(c(1:m),index1),size=1950,replace=F)
      index3<-sample(c(index1),size=50,replace=F)
      beta1[index1]<-coef[index1,1]
      delta[index3,1]<-coef[index3,2]
      delta[index2,1]<-coef[index2,2]
      beta=beta1
      y<-matrix(NA,n,m)
      y<-x%*%(t(beta))+3*z%*%(t(delta))+e
      ...
      {
        measure<-Get.measure(Y=y,X=x,Hidden=NULL,Coef=beta1) 
        #Z_the[iter,k,h,1,1]=0
        Z_cor[iter,k,h,1,1]=summary(lm(z~1))$r.squared
      }
      ###MSE measure 
      ...
      ctl0<-c(sample(which(beta1!=0&delta[,1]!=0),size=50,replace=F),
              sample(which(beta1==0&delta[,1]!=0),size=100,replace=F),
              sample(which(beta1==0&delta[,1]==0),size=100,replace=F),
              sample(which(beta1!=0&delta[,1]==0),size=10,replace=F))
      ctl<-rep(F,m)
      ctl[ctl0]<-T;sum(ctl)
...
save.image("Data-Case0_v14.RData")
