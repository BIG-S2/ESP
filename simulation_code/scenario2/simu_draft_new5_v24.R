...
source('/proj/tengfei/user/xifeng/isva/simu/rPCA-function-Sep16.R')
source('/proj/tengfei/user/xifeng/isva/simu/lfmm_main.R')
...
for (iter in 1:n.run){if(iter %%1==0) {print(10000+iter)}
  for (k in 1:1){
    for (h in 1:4){
      ...
      delta<-matrix(0,m,1)
      Sigma <- matrix(c(1,0.1,0.1,1),2,2)   #B D correlation
      coef<-mvrnorm(n=m,mu=c(0,0),Sigma=Sigma)
      cor(coef[,1],coef[,2])
      index1<-sample(c(1:m),size=1000,replace=F)  #sparsity
      index2<-sample(setdiff(c(1:m),index1),size=1900,replace=F)
      index3<-sample(c(index1),size=100,replace=F)
...
save.image("Data-Case5_v24.RData")
